...
class ClinetBot(object):
    ...
    def life():
        try:
            return joblib.load('life.pkl') == "on"
        except Exception as e:
            print(f"Wrong:life.pkl do not exist{str(e)}")
            joblib.dump("off", 'life.pkl')
            return False

    def run(self, pushService, config):

        if not config.ClientBot.statu:
            return
        ...
        def replay(message, items=None):
            ...
            if str(userID) == config.ClientBot.owner:
                try:
                    ...
                    bot.reply_to(message, f"Wrong:{str(e)}")
            ...
            if ids:
                if count.get(userID) < 30:
                    if ClinetBot.life():
                        rssBvidItem = ids
                        bot.reply_to(message, f"OK,NewTask:{str(rssBvidItem)}")
                        if items:
                            for k, v in items.items():
                                rssBvidItem.append(biliParse().biliIdGet(str(v))[0])
                        try:
                            if len(rssBvidItem) != 0:
